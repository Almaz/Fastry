...
function getLettersForLocale(locale) {
    const latinChars = [
...
function initLetterGlitch(root) {
    if (!(root instanceof HTMLElement))
        return;
    if (root.dataset.letterGlitchInit === 'true')
        return;
...
export function initAllLetterGlitches() {
...
function scheduleInit() {
    ...
    const prefersReducedMotion = window.matchMedia
        ? window.matchMedia('(prefers-reduced-motion: reduce)').matches
        : false;
    if (!hasDesktopPointer || prefersReducedMotion)
        return;
    initAllLetterGlitches();
}
if (typeof window !== 'undefined') {
    window.__letterGlitchRuntimeLoaded = true;
    scheduleInit();
