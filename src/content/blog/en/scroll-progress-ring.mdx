...
description: "Fastry's back-to-top button now has a circular SVG progress ring that fills as you scroll. It's brand-coloured, theme-aware, and runs entirely in CSS and a small inline script."
...
Fastry's back-to-top button has been upgraded with a circular scroll-progress ring. As you scroll down the page, the ring fills clockwise around the button — giving the reader a visual sense of how far through the page they are. Clicking the button still scrolls back to the top.
...
Fastry uses Astro View Transitions for client-side page navigation. Because the back-to-top script is `is:inline` and runs once on first load, its scroll listener would otherwise accumulate across page navigations. The script removes the listener before each swap:
...
The scroll progress ring joins two other scroll-aware UX features in Fastry:
