---
title: "Fastry Configuration — Every Toggle, Theme, and Layout Option Explained"
description: "A complete walkthrough of Fastry's configuration options: 12 colour themes, OKLCH colours, typography, radius and shadow tokens, header styles, dark mode, and more."
...
imageAlt: "Feature overview graphic for Fastry on a dark background"
uid: "configuration-guide"
---

Fastry ships configured with sensible defaults. This post documents every meaningful option you can change — what it does, where to find it, and what value to set.
...
Fastry uses a 3-state colour-mode system — **System / Light / Dark** — instead of a binary toggle. Defaults are declared on the `<html>` element in `src/layouts/BaseLayout.astro`:
...
Fastry ships with **12 colour themes**. All 12 are shown as coloured swatches in the header via the `ThemeSelector` component. Clicking a swatch switches the active theme instantly — no file edits, no rebuild. The logo badge, blog image gradients, and every brand colour on the page update live together.
...
All colour values in Fastry use the OKLCH colour space: `oklch(lightness% chroma hue)`.
...
Fastry has two header shapes. The landing and marketing layouts use a floating capsule:
...
When one post links to another, hard-coding the URL — `[text](/blog/some-slug)` — is fragile: rename the target file and every link to it breaks silently. Fastry gives each post an optional **canonical id** so you can link by a stable identifier instead of the slug.
...
If you like Fastry, a [star on GitHub](https://github.com/hansmartensdev/astro-rocket) helps other developers find it. Takes two seconds.
