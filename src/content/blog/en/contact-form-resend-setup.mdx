...
description: "Fastry's contact form is wired up but needs three things to actually deliver email: a Resend account, a verified domain, and one environment variable on Vercel."
...
[Fastry](https://astrorocket.dev) ships with a fully working contact form — validation, honeypot spam protection, error handling, the lot. The one thing it does not include is an email service account, because that belongs to you. Getting the form to actually deliver requires three external pieces: a [Resend](https://resend.com) account, a verified sending domain, and one environment variable on Vercel. Here is exactly how to connect them.

**[Resend](https://resend.com)** is a developer-focused email delivery service. You send email through their API and they handle the infrastructure — deliverability, authentication, and sending reputation. It has a generous free tier and is what Fastry uses under the hood.
