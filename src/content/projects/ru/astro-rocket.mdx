...
image: "../../../assets/projects/hans-martens-dev-astro-rocket.webp"
imageAlt: "Fastry — open-source Astro 6 тема."
---
...
[Fastry](https://astro.build/themes/details/astro-rocket/), created by Hans Martens — web designer and developer from Veghel, the Netherlands — is a production-ready Astro 6 starter that ships as a **complete, styled website**, not a blank slate. Clone it, update the text, go live. Everything else is already done.
...
The theme switcher is one of Fastry's most satisfying details. Twelve colour themes — **Orange, Amber, Lime, Emerald, Teal, Cyan, Sky, Blue, Indigo, Violet, Purple, and Magenta** — toggle instantly from a pill in the header. Click a swatch and every button, link, badge, progress bar, scroll-progress arc, logo monogram, blog cover gradient, and accent on the page updates in a single frame. No waiting, no code changes, no deploy.

The secret is CSS custom properties scoped to `data-theme` attributes on `<html>`. The browser does all the work — the server never needs to know. The full token system — how colours, spacing, and typography are wired together — is covered in [How Fastry's Design System Works](/blog/design-system-color-tokens).
...
Most starters ship three meta tags and call it SEO. Fastry ships the complete implementation:
...
The full setup — what each piece does, how to configure it, and what rich results it unlocks — is in [SEO in Fastry](/blog/seo-in-astro-rocket).

## Internationalization — Native, Opt-In

As of **v1.3.0**, internationalization is built into Fastry itself — no upstream CLI, no project regeneration, no content restructure. Flip `enabled: true` in `src/config/i18n.config.ts` and the theme switches on:
...
A full technical breakdown of every animation layer is in [Animations in Fastry](/blog/animations-in-astro-rocket).
...
Fastry ships with first-party configuration for **Vercel**, **Netlify**, and **Cloudflare Pages** — `vercel.json`, `netlify.toml`, and `wrangler.toml` are all in the repo, each with security headers pre-configured. CI runs lint, type-check, and build on every push via GitHub Actions.
...
Fastry is open source under the MIT licence and submitted to the official Astro themes directory. It's a fork of and a tribute to [Velocity](https://github.com/southwellmedia/velocity) by Southwell Media — the foundation it builds on. If it saves you a week of setup, [a star on GitHub](https://github.com/hansmartensdev/Astro-Rocket) is the best way to say thanks.
